<div class='row'>
    ...
    <div class='input-field col s12 m6'>
        ...
        <label for='login'><i class='fa fa-user'></i> Login</label>
    </div>

    <div class='input-field col s12 m6'>
        <input class='validate' required type='password' name='password' id='password' {{ isset($turmas->password) ? 'readonly' : ''}} value='{{ isset($turmas->password) ? '12345678' : ''}}'>
        <label for='password'><i class='fa fa-lock'></i> Senha</label>
    </div>
    <div class='input-field col s12 m6'>
        <input class='validate' required type='password' name='confpassword' id='confpassword' {{ isset($turmas->password) ? 'readonly' : ''}} value='{{ isset($turmas->password) ? '12345678' : ''}}'>
        ...
        <select id='curso_id' required name='curso_id'>
            @if(isset($turmas->curso_id))
                @foreach($cursos as $curso)
                    <option value="{{ $curso->id }}" {{ ($curso->id == $turmas->curso_id) ? 'selected' : ''}}>{{ $curso->name }}</option>
                @endforeach
            @else
